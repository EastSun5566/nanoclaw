...
3. `cd nanoclaw`
4. `claude` (or `opencode`, `amp`, `codex`)

</details>

Then run `/setup`. Your agent handles everything: dependencies, authentication, container setup and service configuration.

> **Note:** Commands prefixed with `/` (like `/setup`, `/add-whatsapp`) are agent skills (stored in `.agents/skills/`). Type them inside your agent's prompt. Supported out of the box by Claude Code, opencode, Amp, and Codex CLI.
...
- Node.js 20+
- A coding agent: [Claude Code](https://claude.ai/download), [opencode](https://opencode.ai), [Amp](https://ampcode.com), or [Codex CLI](https://github.com/openai/codex)
